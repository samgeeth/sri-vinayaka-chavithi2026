...
export const TimelineSection: React.FC<TimelineSectionProps> = ({ onImageSelect }) => {
  ...
  const overallProgress = Math.round(
    WORK_UPDATES.reduce((acc, curr) => acc + curr.progress, 0) / WORK_UPDATES.length
  );
          ...
          <div>
            ...
            <BlurReveal delay={0.3}>
              <p className="text-gray-400 text-base sm:text-lg max-w-2xl mt-3 font-sans">
                Real-time accountability of every stage undertaken by Maraigudem youth volunteers to prepare the sacred grounds.
              </p>
            </BlurReveal>
          </div>

          {/* Overall Progress Widget with Animated Counter */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              {WORK_UPDATES.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="font-display text-base font-bold text-white mb-2 line-clamp-1">
                      {item.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between text-[11px] text-gray-400 font-sans">
                      ...
                      <span className="font-semibold text-gray-300">{item.progress}%</span>
                    </div>
                  </motion.div>
                );
              })}
          ...
          <div className="lg:col-span-7">
            <AnimatePresence mode="wait">
              {(() => {
                const active = WORK_UPDATES.find((w) => w.day === selectedDay) || WORK_UPDATES[0];
                return (
                  <motion.div
                    key={active.day}
                  ...
                  >
                    <div>
                      ...
                      <div>
                        <div className="text-xs uppercase tracking-wider font-semibold text-gray-400 mb-3 flex items-center gap-1.5 font-sans">
                          ...
                          On-Site Documentation Photos (Click to Zoom)
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          {active.photos.map((photo, pIdx) => (
                            <div
                              key={pIdx}
                              onClick={() => onImageSelect(photo, `${active.title} - Day ${active.day}`)}
                              className="relative h-44 sm:h-52 rounded-2xl overflow-hidden cursor-pointer group border border-white/10 shadow-lg"
                            >
                              <img
                                src={photo}
                                alt={active.title}
                              ...
                              />
                              <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent opacity-60 group-hover:opacity-25 transition-opacity" />
                              <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-xs text-white">
                                <span className="font-semibold truncate">Work Photo #{pIdx + 1}</span>
                                <span className="bg-black/70 px-2.5 py-0.5 rounded-full text-[10px] backdrop-blur-md border border-white/10">
                                  Expand
                                </span>
                              </div>
                            </div>
                    ...
                    {/* Footer lead tag */}
                    <div className="mt-8 pt-5 border-t border-white/10 flex items-center justify-between text-xs text-gray-400 font-sans">
                      <span>Assigned Squad: <strong className="text-white">{active.lead}</strong></span>
                      <span className="text-[#FF8C00] font-semibold flex items-center gap-1">
                        Maraigudem Youth Verified <ShieldCheck className="w-3.5 h-3.5" />
                      </span>
                    </div>
                  </motion.div>
                );
